...
// Variável global para armazenar uma string
...
void send_hello_message(esp_gatt_if_t gatts_if, uint16_t conn_id) {
    char hello_message[] = "Olá, mundo!";

    // Verifica se o handle de característica está correto antes de tentar enviar
    if (spp_handle_table[SPP_IDX_SPP_DATA_RECV_VAL] != 0) {
        if (is_connected) {  // Verifica se a conexão está ativa
            esp_err_t err = esp_ble_gatts_send_indicate(gatts_if, conn_id, spp_handle_table[SPP_IDX_SPP_DATA_RECV_VAL],
                                                        strlen(hello_message), (uint8_t *)hello_message, false);
            if (err == ESP_OK) {
                ESP_LOGI(GATTS_TABLE_TAG, "Mensagem 'Olá' enviada com sucesso! Conn_ID: %d, Tamanho: %d", conn_id, strlen(hello_message));
            } else {
                ESP_LOGE(GATTS_TABLE_TAG, "Erro ao enviar mensagem: %s", esp_err_to_name(err));
            }
        } else {
            ESP_LOGE(GATTS_TABLE_TAG, "Não há conexão ativa para enviar a mensagem.");
        }
    } else {
        ESP_LOGE(GATTS_TABLE_TAG, "Handle de característica inválido! A característica não foi configurada corretamente.");
    }
}
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	...
    	case ESP_GATTS_READ_EVT:
            ...
            ESP_LOGI(GATTS_TABLE_TAG, "res : %d", res);
            if(res == 4){
            ...
            rsp.attr_value.value[3] = 'd';
                ...
                esp_err_t error = esp_ble_gatts_send_response(gatts_if, p_data->read.conn_id, p_data->read.trans_id,
                                      ESP_GATT_OK, &rsp);
                ESP_LOGI(GATTS_TABLE_TAG, "error : %d", error);
            }
       	 break;
    	case ESP_GATTS_WRITE_EVT: {
            ...
            if(p_data->write.is_prep == false){
                ...
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    strncpy(global_string, (char*)(p_data->write.value), p_data->write.len);
#endif
                }else{
                    //TODO:
                }
    ...
    break;
    	...
    	case ESP_GATTS_CONNECT_EVT:
            ...
            ESP_LOGI(GATTS_TABLE_TAG, "Dispositivo conectado!");
            send_hello_message(gatts_if, p_data->connect.conn_id);
#ifdef SUPPORT_HEARTBEAT
    	    uint16_t cmd = 0;
    ...
    while(1){
        ...
        if(global_string[0]!=0 && (strstr(global_string, "gravar") != NULL)){
            ...
            esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id,
                                                                ...
                                                                strlen(global_string), (uint8_t *)global_string, false);
            ...
            //armazenaria o codigo recebido na variavel que sera usada para transmissao
            //
            strcpy(my_string,"10");
            my_integer = atoi(global_string);
            ESP_LOGI(GATTS_TABLE_TAG, "Valor de global_string convertido para inteiro: %d", my_integer);
            esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id,
                spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],
                strlen(my_string), (uint8_t *)my_string, false);
